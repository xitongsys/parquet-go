...
//Encode dict chunk
func EncodeDictChunk(chunk *Chunk) {
	if chunk == nil {
		return
	}
	if ln := len(chunk.Pages); ln <= 0 {
		return
	}
	table := NewTableFromTable(chunk.Pages[0].DataTable)
	table.Type = parquet.Type_INT32

	dictMap := make(map[interface{}]ParquetType.INT32)
	index := -1
	dictMap[nil] = -1
	dictTable := NewTableFromTable(chunk.Pages[0].DataTable)
	pageSize := chunk.Pages[0].PageSize
	...
	chunk = PagesToChunkWithDictHead(pages)
}
